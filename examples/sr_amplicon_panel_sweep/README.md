# GTDB panel reinterpretation vs a custom database

Asks how much is lost when amplicon reads **cannot be re-mapped against the community**.
The reads were classified against GTDB, and the community is known to come from a panel:
...
- **`gtdb_panel`**: keep the GTDB labels and reinterpret them through the panel.
  superresolution-amplicon measures a rectangular kernel (panel V4 amplicons x GTDB
  labels) by simulating reads from the panel and mapping them against GTDB. It then
...
- the **grids** come from `examples/sr_amplicon_param_sweep`: `grid:` axes expanded into
  `sr_settings:`;
- **map GTDB once, sweep afterwards** comes from `examples/sr_amplicon_gtdb_sweep`:
  phase 1's GTDB classification is handed back as `mseq:`.
...
| `gtdb_panel` | `kernel` | `trained` (simulates with the model the reads were made with), `flat` |
...
Every `gtdb_panel` point gets `panel: <database.name>` from the generator. There is no
unreinterpreted GTDB arm: its profiles would name GTDB genomes, which cannot be scored
against panel ids, and raw GTDB genome calls are not meant to be read that way.
...
- `config.yaml` → `gtdb.path`: a directory holding `<gtdb.name>_ssu.sr_refs.fasta`
  (headers `{genome_id}|{copy}|{orig}`), built once and out of band. The sequences must
...
1. `generate_samplesheet.py` → `samplesheet.yaml` (+ `genomes/sample_NN.amplicon_16s.csv`),
   ...
   maps each depth against GTDB **once** under `gtdb.map_setting` (align exact-hash, cheap
   at GTDB scale). It publishes `profiling/sr/<id>.map.obs.mseq.gz` per dir, and
...
2. `generate_sweep_samplesheet.py` → `sweep_samplesheet.yaml`, then `--step profile`. There
   ...
   custom grid. The `gtdb_panel` row has `database: gtdb_ssu_r232`, phase 1's `mseq:`,
...
4 `custom` + 6 `gtdb_panel` points, which is **400 profiles**. The expensive parts:

- **GTDB read mapping, once**, in phase 1. No phase-2 setting re-maps the reads.
- **2 custom matrices** over 23 references. The two `prior` points share each one.
- **6 panel kernels**, one per `gtdb_panel` point. The benchmark batches every dir of a
  ...
  panel amplicons simulated reads mapped against GTDB, once per point, whatever the number of
  dirs. Adding a `gtdb_panel` point adds a kernel; adding a `custom` inference point does
  not add a matrix.
...
results/sr_amplicon_panel_sweep/
  ...
  S01.amplicon_16s.515-YF-806BR/
    ...
    S01.amplicon_16s.515-YF-806BR.gtdb_panel.trained.nogate.s3k.sr_profile.tsv
    ...
...
the custom database and 0.015 for `gtdb_panel.trained.nogate`. Flat kernels were no better
than GTDB home labels alone. That sweep used equal abundances and a strain split
(superresolution-amplicon `dev/panel_reinterpretation.md`), so these NB communities are a
different test, not a replication.
...
- Phase 1's composition (`<id>.map.sr_profile.tsv`) is an unscored by-product of the
  GTDB mapping. It is inferred over GTDB genomes.
