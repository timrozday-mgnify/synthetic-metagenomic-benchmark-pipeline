...
Reuses the same config.yaml as generate_samplesheet.py: it emits the same `databases:`
block (so the pipeline builds the DB) and one `samples:` row per
...
DEFAULT_RESULTS_DIR = HERE.parent.parent / "results" / "abundance_nb_sample"
