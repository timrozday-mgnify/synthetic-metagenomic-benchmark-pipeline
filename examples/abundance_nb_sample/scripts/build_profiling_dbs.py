...
The DEFAULT path does NOT need this script: generate_samplesheet.py emits a `databases:`
...
def tax_string(genome_id):
    return nc.taxonomy(BY_ID[genome_id])
...
params {{
    sylph_databases = [
        {db_name}: [ syldb: '{syldb_path}', label: 'abundance_nb_sample community' ],
    ]
}}
""")
    if mapseq_paths:
...
params {{
    mapseq_databases {{
        {db_name} {{
            ...
            label = 'abundance_nb_sample community'
            run_otu = true
            run_asv = false
        }}
    }}
...
def main():
    ...
    cfg = nc.load_config(args.config)
