...
def _selfcheck():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # sr_amplicon needs `ssu:` (present here) but not taxonomy; sr_shotgun needs
        # `genome:` (absent) -> validation exits naming the missing field.
        p.write_text(cfg_text.replace("profilers: [aap]", "profilers: [sr_amplicon]"))
        seqs_sr = database_block(load_config(p))["community"]["sequences"]
        assert "ssu" in seqs_sr[0] and "taxonomy" not in seqs_sr[0], seqs_sr
        assert "genome" not in seqs_sr[0], seqs_sr
