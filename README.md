...
`.syldb` for `profilers: [sylph]`, a mapseq quartet for `profilers: [aap]` — once per run,
...
- sample: S3.27F-534R
  ...
  benchmark_dir: results/S3.27F-534R
  database: community_v4
  primers:             # the ONE pair these reads were amplified with
    - {pair_id: 27F-534R, forward: AGAGTTTGATCMTGGCTCAG, reverse: ATTACCGCGGCTGCTGG}
```

`primers` matters only for `sr_amplicon`: the nested pipeline cuts its reference
amplicons with its own in-silico PCR, defaulting to V4 (515F/806R), so reads from
another region match nothing unless the pair is given. It also separates the
...
Set `profilers` per sample to profile the generated reads and drop each predicted
profile next to `truth.tsv`.

### Benchmarking several profilers at once

To compare methods, list them all on the sample — reads are generated once and each
method profiles them, writing its own file into the same benchmark dir:

```yaml
- sample: S1
  profilers: [sylph, sr_shotgun]  # both, against the same reads
  database: community             # one collection can serve both
```

Giving `<outdir>/S1/S1.sylph_profile.tsv` and `<outdir>/S1/S1.sr_profile.tsv`
alongside `S1.truth.tsv`. Set the list globally with `--profilers sylph,sr_shotgun`
(or a top-level `profilers:` key in the samplesheet); the per-sample column wins.
...
`aap` runs the EBI-Metagenomics amplicon-analysis-pipeline via a nested
...
`sr_shotgun` (WGS) and `sr_amplicon` (16S) run the two
